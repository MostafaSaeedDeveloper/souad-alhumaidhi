...
@php($title = $item->exists ? 'تعديل إنجاز' : 'إضافة إنجاز')

@section('content')
<form method="POST" action="{{ $item->exists ? route('admin.achievements.update', $item) : route('admin.achievements.store') }}" enctype="multipart/form-data" class="stat-card">
  @csrf
  ...
  <div class="row g-3">
    ...
    <div class="col-md-4">
      ...
      <input type="text" name="year" value="{{ old('year', $item->year) }}" class="form-control">
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-4">
      ...
      <input type="text" name="icon" value="{{ old('icon', $item->icon) }}" class="form-control" placeholder="bi-award">
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-3 d-flex align-items-end">
      <div class="form-check">
        ...
        <label class="form-check-label" for="ift">مميز في الرئيسية</label>
      </div>
    </div>
  </div>
  <button type="submit" class="btn btn-dark mt-4">حفظ</button>
  <a href="{{ route('admin.achievements.index') }}" class="btn btn-outline-secondary mt-4">إلغاء</a>
</form>
@endsection
